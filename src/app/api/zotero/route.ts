...
export async function POST(req: Request) {
  ...
  const zotero = async (key: string, userid: string, collectionid: string) => {
    ...
    for (let i = 0; i < response.raw.length; i++) {
      if (
        !response.raw[i].data.title ||
        !response.raw[i].data.abstractNote ||
        !response.raw[i].data.creators[0]
      ) {
        continue;
      }
      let authors = "";
      for (let j = 0; j < response.raw[i].data.creators.length; j++) {
        authors +=
          response.raw[i].data.creators[j].lastName +
          ", " +
          response.raw[i].data.creators[j].firstName +
          "; ";
      }
      authors += "et al.";
      let item = {
  ...
  try {
    ...
    try {
      items = await zotero(
        container.resolve("zotero-key"),
        container.resolve("zotero-userid"),
        zoteroCollection
      );
    } catch (e) {
